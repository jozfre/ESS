...
// Get event details
$sql = "SELECT e.*, s.spaceName, u.orgName 
        FROM event e
      ...
      var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events: <?php
                $calendarEvents = array_map(function ($event) {
                  return [
                    'id' => $event['eventID'],
                    'title' => "{$event['eventName']} - {$event['spaceName']} - {$event['eventType']}",
                    'start' => $event['eventDate'] . 'T' . $event['eventTimeStart'],
                    'end' => $event['eventDate'] . 'T' . $event['eventTimeEnd'],
                    'backgroundColor' => $event['eventType'] === 'Islamic Talks' ? 'green' : ($event['eventType'] === 'Nikah/Wedding' ? 'pink' : ($event['eventType'] === 'Class' ? '#0056b3' : 'black')),
                    'textColor' => $event['eventType'] === 'Nikah/Wedding' ? 'black' : 'white'
                  ];
                }, $events);
                echo json_encode($calendarEvents);
                ?>,
        eventClick: function(info) {
          window.location.href = `../event/view-event.php?eventID=${info.event.id}`;
        }
      });
      calendar.render();
    });
