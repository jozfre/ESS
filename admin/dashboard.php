...
$sqlPending = "SELECT COUNT(*) as pending FROM request 
               WHERE approvalStatus IS NULL AND isDeleted = 0";
$resultPending = mysqli_query($conn, $sqlPending);
$pendingCount = mysqli_fetch_assoc($resultPending)['pending'];

// Get events this week count
$sqlWeek = "SELECT COUNT(*) as weekEvents 
            FROM event 
            WHERE eventDate BETWEEN DATE_SUB(CURDATE(), INTERVAL WEEKDAY(CURDATE()) DAY) 
            AND DATE_ADD(CURDATE(), INTERVAL 6 - WEEKDAY(CURDATE()) DAY)
            AND isDeleted = 0";
$resultWeek = mysqli_query($conn, $sqlWeek);
$weekCount = mysqli_fetch_assoc($resultWeek)['weekEvents'];

// Get events this month count
$sqlMonth = "SELECT COUNT(*) as monthEvents 
             FROM event 
             WHERE MONTH(eventDate) = MONTH(CURRENT_DATE()) 
             AND YEAR(eventDate) = YEAR(CURRENT_DATE())
             AND isDeleted = 0";
$resultMonth = mysqli_query($conn, $sqlMonth);
$monthCount = mysqli_fetch_assoc($resultMonth)['monthEvents'];
?>
